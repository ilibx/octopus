...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "octopus-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "octopus") {
...
// (~/.octopus/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	...
	globalSkillPath := filepath.Join(tmpHome, ".octopus", "skills", "global-skill", "SKILL.md")
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	...
	t.Setenv("OCTOPUS_BUILTIN_SKILLS", builtinRoot)
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "octopus") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "octopus-bench-*")
	defer os.RemoveAll(tmpDir)
