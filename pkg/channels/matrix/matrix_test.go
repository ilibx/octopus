...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("octopus")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@octopus hello", want: true},
		{text: "hi @octopus:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0boctopus\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@octopus:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@octopus:matrix.org hello", want: "hello"},
		{in: "@octopus, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@octopus:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@octopus:matrix.org">Octopus</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40octopus%3Amatrix.org">Octopus</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@octopus:matrix.test"), "")
	if err != nil {
