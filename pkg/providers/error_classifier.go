...
// Error patterns organized by FailoverReason, matching Octopus production (~40 patterns).
var (
...
func ClassifyError(err error, provider, model string) *FailoverError {
	...
	// Message pattern matching (priority order from Octopus).
...
// Priority order matters (from Octopus classifyFailoverReason).
