...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Octopus.
...
10. [Creating a New Provider in Octopus](#creating-a-new-provider-in-octopus)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<OctopusConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Octopus codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.octopus/auth.json`:
...
## Creating a New Provider in Octopus

Octopus providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/octopus/cmd_auth.go`:
...
octopus auth login --provider your-provider
...
octopus agent -m "Hello" --model your-model
```
...
### "Token expired"
- Refresh OAuth tokens: `octopus auth login --provider antigravity`
...
- Check auth profile storage: `~/.octopus/auth.json`
- Re-run `octopus auth login --provider antigravity`
