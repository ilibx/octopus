# Using Antigravity Provider in Octopus

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Octopus.
...
```bash
octopus auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where Octopus is waiting.

Octopus will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.octopus/config.json` or override it via the CLI:

```bash
# Override for a single command
octopus agent -m "Hello" --model claude-opus-4-6-thinking
```
...
2.  **Authentication persistence**: 
    ...
    ```bash
    scp ~/.octopus/auth.json user@your-server:~/.octopus/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. Octopus will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `octopus auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
