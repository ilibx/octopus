...
<h1>Octopus: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 Octopus は [octopus](https://github.com/HKUDS/octopus) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。

⚡️ $10 のハードウェアで 10MB 未満の RAM で動作：Octopus より 99% 少ないメモリ、Mac mini より 98% 安い！
...
## 📢 ニュース
2026-02-09 🎉 Octopus リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！

## ✨ 特徴

🪶 **超軽量**: メモリフットプリント 10MB 未満 — Octopus のコア機能より 99% 小さい。
...
### 🐜 革新的な省フットプリントデプロイ
Octopus はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/ilibx/octopus/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに Octopus を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `OCTOPUS_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、Octopus はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で Octopus と会話できます
...
> **Docker Compose**: Gateway HTTP サーバーは共有の `127.0.0.1:18790` で Webhook を提供します。ホストからアクセスするには `octopus-gateway` サービスに `ports: ["18790:18790"]` を追加してください。
...
Octopus は3種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.octopus/config.json`

### 環境変数

環境変数を使用してデフォルトのパスを上書きできます。これは、ポータブルインストール、コンテナ化されたデプロイメント、または octopus をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICOCLAW_CONFIG` | 設定ファイルへのパスを上書きします。これにより、octopus は他のすべての場所を無視して、指定された `config.json` をロードします。 | `~/.octopus/config.json` |
| `PICOCLAW_HOME`   | octopus データのルートディレクトリを上書きします。これにより、`workspace` やその他のデータディレクトリのデフォルトの場所が変更されます。          | `~/.octopus`             |

**例：**

```bash
# 特定の設定ファイルを使用して octopus を実行する
# ワークスペースのパスはその設定ファイル内から読み込まれます
PICOCLAW_CONFIG=/etc/octopus/production.json octopus gateway

# すべてのデータを /opt/octopus に保存して octopus を実行する
# 設定はデフォルトの ~/.octopus/config.json からロードされます
# ワークスペースは /opt/octopus/workspace に作成されます
...
Octopus は設定されたワークスペース（デフォルト: `~/.octopus/workspace`）にデータを保存します：
...
Octopus はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.octopus/workspace` | エージェントの作業ディレクトリ |
...
Octopus は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
| `volcengine`             | LLM(Volcengine 直接)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus)           |
...
> **新機能！** Octopus は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [キーを取得](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus) |
...
```
> OAuth認証を設定するには、`octopus auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、Octopus が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `octopus onboard` | 設定＆ワークスペースの初期化 |
| `octopus agent -m "..."` | エージェントとチャット |
| `octopus agent` | インタラクティブチャットモード |
| `octopus gateway` | ゲートウェイを起動 |
| `octopus status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。Octopus は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`octopus gateway` が 1 つだけ実行されていることを確認してください。
