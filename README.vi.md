...
<h1>Octopus: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
🦐 **Octopus** là trợ lý AI cá nhân siêu nhẹ, lấy cảm hứng từ [octopus](https://github.com/HKUDS/octopus), được viết lại hoàn toàn bằng **Go** thông qua quá trình "tự khởi tạo" (self-bootstrapping) — nơi chính AI Agent đã tự dẫn dắt toàn bộ quá trình chuyển đổi kiến trúc và tối ưu hóa mã nguồn.

⚡️ **Cực kỳ nhẹ:** Chạy trên phần cứng chỉ **$10** với RAM **<10MB**. Tiết kiệm 99% bộ nhớ so với Octopus và rẻ hơn 98% so với Mac mini!
...
>
> * **KHÔNG CÓ CRYPTO:** Octopus **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[octopus.io](https://octopus.io)**, website công ty là **[sipeed.com](https://sipeed.com)**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** Octopus đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** Octopus gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.


## 📢 Tin tức

2026-02-16 🎉 Octopus đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! Octopus đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 Octopus đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển Octopus.
...
2026-02-09 🎉 Octopus chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 Octopus, Lên Đường!

## ✨ Tính năng nổi bật

🪶 **Siêu nhẹ**: Bộ nhớ sử dụng <10MB — nhỏ hơn 99% so với Octopus (chức năng cốt lõi).
...
Octopus có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/ilibx/octopus/releases).
...
Bạn cũng có thể chạy Octopus bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `OCTOPUS_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm octopus-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.octopus/config.json`. Lấy API key: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). Tìm kiếm web là **tùy chọn** — lấy [Tavily API](https://tavily.com) miễn phí (1000 truy vấn/tháng) hoặc [Brave Search API](https://brave.com/search/api) (2000 truy vấn/tháng).
...
**2. Cấu hình** (`~/.octopus/config.json`)
...
> `request_timeout` là tùy chọn và dùng đơn vị giây. Nếu bỏ qua hoặc đặt `<= 0`, Octopus sẽ dùng timeout mặc định (120s).
...
```bash
octopus agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với Octopus qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
Octopus hỗ trợ ba loại tích hợp WeCom:
...
Kết nối Octopus với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.octopus/config.json`

### Biến môi trường

Bạn có thể ghi đè các đường dẫn mặc định bằng cách sử dụng các biến môi trường. Điều này hữu ích cho việc cài đặt di động, triển khai container hóa hoặc chạy octopus như một dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường dẫn mặc định        |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICOCLAW_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Điều này trực tiếp yêu cầu octopus tải file `config.json` nào, bỏ qua tất cả các vị trí khác. | `~/.octopus/config.json` |
| `PICOCLAW_HOME`   | Ghi đè thư mục gốc cho dữ liệu octopus. Điều này thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.octopus`             |

**Ví dụ:**

```bash
# Chạy octopus bằng một file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
PICOCLAW_CONFIG=/etc/octopus/production.json octopus gateway

# Chạy octopus với tất cả dữ liệu được lưu trữ trong /opt/octopus
# Cấu hình sẽ được tải từ ~/.octopus/config.json mặc định
# Workspace sẽ được tạo tại /opt/octopus/workspace
...
Octopus lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.octopus/workspace`):
...
Octopus chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.octopus/workspace` | Thư mục làm việc của agent |
...
Octopus có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
| `volcengine`             | LLM(Volcengine trực tiếp)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus)           |
...
> **Tính năng mới!** Octopus hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [Lấy Khóa](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus) |
...
```
> Chạy `octopus auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
Định cấu hình nhiều endpoint cho cùng một tên mô hình—Octopus sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `octopus onboard` | Khởi tạo cấu hình & workspace |
| `octopus agent -m "..."` | Trò chuyện với agent |
| `octopus agent` | Chế độ chat tương tác |
| `octopus gateway` | Khởi động gateway (cho bot chat) |
| `octopus status` | Hiển thị trạng thái |
| `octopus cron list` | Liệt kê tất cả tác vụ định kỳ |
| `octopus cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

Octopus hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.octopus/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. Octopus sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.octopus/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `octopus gateway` chạy tại một thời điểm.
