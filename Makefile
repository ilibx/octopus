...
CONFIG_PKG=github.com/ilibx/octopus/pkg/config
...
WORKSPACE_DIR?=$(OCTOPUS_HOME)/workspace
...
## build: Build the octopus binary for current platform
build: generate
...
## build-launcher: Build the octopus-launcher (web console) binary
build-launcher:
	@echo "Building octopus-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/octopus-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf octopus-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/octopus-launcher
	@echo "Build complete: $(BUILD_DIR)/octopus-launcher"
...
## build-all: Build octopus for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install octopus to system and copy builtin skills
install: build
...
## uninstall: Remove octopus from system
uninstall:
...
## uninstall-all: Remove octopus and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(OCTOPUS_HOME)
	@echo "Removed workspace: $(OCTOPUS_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build octopus-agent octopus-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build octopus-agent octopus-gateway
...
## docker-run: Run octopus gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run octopus gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run octopus agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm octopus-agent

## docker-run-agent-full: Run octopus agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm octopus-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi octopus:latest octopus:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "octopus Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.octopus/workspace)"
