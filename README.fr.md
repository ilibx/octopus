<div align="center">
  ...
  <h1>Octopus : Assistant IA Ultra-Efficace en Go</h1>
...
🦐 **Octopus** est un assistant personnel IA ultra-léger inspiré de [octopus](https://github.com/HKUDS/octopus), entièrement réécrit en **Go** via un processus d'auto-amorçage (self-bootstrapping) — où l'agent IA lui-même a piloté l'intégralité de la migration architecturale et de l'optimisation du code.

⚡️ **Extrêmement léger :** Fonctionne sur du matériel à seulement **10$** avec **<10 Mo** de RAM. C'est 99% de mémoire en moins qu'Octopus et 98% moins cher qu'un Mac mini !
...
>
> * **PAS DE CRYPTO :** Octopus n'a **AUCUN** token/jeton officiel. Toute annonce sur `pump.fun` ou d'autres plateformes de trading est une **ARNAQUE**.
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[octopus.io](https://octopus.io)**, et le site de l'entreprise est **[sipeed.com](https://sipeed.com)**.
> * **Attention :** De nombreux domaines `.ai/.org/.com/.net/...` sont enregistrés par des tiers et ne nous appartiennent pas.
> * **Attention :** Octopus est en phase de développement précoce et peut présenter des problèmes de sécurité réseau non résolus. Ne déployez pas en environnement de production avant la version v1.0.
> * **Note :** Octopus a récemment fusionné de nombreuses PR, ce qui peut entraîner une empreinte mémoire plus importante (10–20 Mo) dans les dernières versions. Nous prévoyons de prioriser l'optimisation des ressources dès que l'ensemble des fonctionnalités sera stabilisé.


## 📢 Actualités

2026-02-16 🎉 Octopus a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! Octopus grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 Octopus a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de Octopus.
...
2026-02-09 🎉 Octopus est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 Octopus, c'est parti !

## ✨ Fonctionnalités

🪶 **Ultra-Léger** : Empreinte mémoire <10 Mo — 99% plus petit que Octopus pour les fonctionnalités essentielles.
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec Octopus. Démarrage rapide :
...
Octopus peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Téléchargez le binaire pour votre plateforme depuis la page des [releases](https://github.com/ilibx/octopus/releases).
...
Vous pouvez également exécuter Octopus avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `OCTOPUS_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm octopus-agent -m "Combien font 2+2 ?"
...
**2. Configurer** (`~/.octopus/config.json`)
...
> `request_timeout` est optionnel et s'exprime en secondes. S'il est omis ou défini à `<= 0`, Octopus utilise le délai d'expiration par défaut (120s).
...
```bash
octopus agent -m "Combien font 2+2 ?"
```
...
Discutez avec votre Octopus via Telegram, Discord, DingTalk, LINE ou WeCom
...
Octopus prend en charge trois types d'intégration WeCom :
...
Connectez Octopus au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.octopus/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de octopus en tant que service système. Ces variables sont indépendantes et contrôlent différents chemins.

| Variable          | Description                                                                                                                             | Chemin par Défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICOCLAW_CONFIG` | Remplace le chemin du fichier de configuration. Cela indique directement à octopus quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.octopus/config.json` |
| `PICOCLAW_HOME`   | Remplace le répertoire racine des données octopus. Cela modifie l'emplacement par défaut du `workspace` et des autres répertoires de données.          | `~/.octopus`             |

**Exemples :**

```bash
# Exécuter octopus en utilisant un fichier de configuration spécifique
...
# Exécuter octopus avec toutes ses données stockées dans /opt/octopus
# La configuration sera chargée à partir du fichier par défaut ~/.octopus/config.json
# Le workspace sera créé dans /opt/octopus/workspace
...
Octopus stocke les données dans votre workspace configuré (par défaut : `~/.octopus/workspace`) :
...
Octopus s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.octopus/workspace` | Répertoire de travail de l'agent |
...
Octopus peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
```bash
octopus agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** Octopus utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [Obtenir Clé](https://www.volcengine.com/activity/codingplan?utm_campaign=Octopus&utm_content=Octopus&utm_medium=devrel&utm_source=OWO&utm_term=Octopus) |
...
```
> Exécutez `octopus auth login --provider anthropic` pour configurer les identifiants OAuth.
...
Configurez plusieurs points de terminaison pour le même nom de modèle—Octopus utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `octopus onboard`        | Initialiser la configuration & le workspace |
| `octopus agent -m "..."` | Discuter avec l'agent                 |
| `octopus agent`          | Mode de discussion interactif         |
| `octopus gateway`        | Démarrer la passerelle                |
| `octopus status`         | Afficher le statut                    |
| `octopus cron list`      | Lister toutes les tâches planifiées   |
| `octopus cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

Octopus prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.octopus/workspace/cron/` et traitées automatiquement.
...
C'est normal si vous n'avez pas encore configuré de clé API de recherche. Octopus fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.octopus/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `octopus gateway` fonctionne à la fois.
